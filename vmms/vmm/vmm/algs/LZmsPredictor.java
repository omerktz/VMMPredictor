...
public class LZmsPredictor
  ...
  public void learn(IntSequence trainingSequence) {
    ...
    for (int i = 0; i < trainingSequence.length(); ++i) {
      symList.add(trainingSequence.intAt(i));
    }
  ...
  public double predict(int symbol, IntSequence context) {
    try {
      ...
      IntSequence seqWithSym = new IntSequence();
      seqWithSym.append(context).append(symbol);
      IntSequence seqNoSym = new IntSequence();
      seqNoSym.append(context);

      return Math.pow( 2.0, - (logEval(new IntSequence(seqWithSym)) - logEval(new IntSequence(seqNoSym))));
    }
  ...
  public double logEval(IntSequence testSequence) {
    try {
      ...
      for (int i = 0; i < testSequence.length(); ++i) {
        symList.add(testSequence.intAt(i));
      }
