...
public class DefaultPSTNode implements PSTNodeInterface, Serializable{
  ...
  public double predict(int ch){
    return nextSymProbability[ch];
  }

  public double predict(IntSequence charSeq){
    double prediction = 1;
    PSTNodeInterface node;
    for(int i=0,test=charSeq.length(); i<test; ++i){
      node = get(charSeq.subSequence(0,i));
      prediction *= node.predict(charSeq.intAt(i));
    }
    return prediction;
  }

  //public double predict(byte b){
  //  return predict(UnsignedByteConverter.value(b));
  //}
  ...
  public double predict(int []bytes){
    double prediction = 1;
    PSTNodeInterface node;
   ...
   *          by seq.
   */
  public PSTNodeInterface get(IntSequence seq){
   ...
   */
  public PSTNodeInterface get(int []byteSeq){
  ...
  public PSTNodeInterface get(int symbol){
         return children[symbol];
  }

  private PSTNodeInterface get(int []bytes, int currentByteIndex){
    if(currentByteIndex==-1){
      return this;
    }
    else{
      int nextSymIndex = bytes[currentByteIndex];//UnsignedByteConverter.value(bytes[currentByteIndex]);
      if(children[nextSymIndex]!=null){
  ...
  public void insert(int symbol, double[] nextSymProbability){
         if(isLeaf){
           isLeaf = false;
           children = new DefaultPSTNode[absize];
         }
  ...
  private String prob2Str(){
    ...
    for(int i=0; i<nextSymProbability.length; ++i){
      if(nextSymProbability[i]!=0){
        sbuff.append(i).append('=').append(nextSymProbability[i]).append(',');
      }
    }
